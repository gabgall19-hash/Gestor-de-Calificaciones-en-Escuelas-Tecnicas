...
export const handlePrintParteDiario = (data, course, scheduleData) => {
  ...
  const renderAttendanceTable = (cols, startIdx) => `
    <table class="attendance-table" style="width: ${rightCount > 0 ? '100%' : '50%'};">
      <colgroup>
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .attendance-container {
            display: grid;
            grid-template-columns: ${rightCount > 0 ? '1fr 1fr' : '1fr'};
            gap: 8px;
            margin-bottom: 25px;
            align-items: start;
          }
      ...
      <body>
        <div class="no-print-toolbar">
          <button class="btn-print" onclick="window.print()">🖨️ IMPRIMIR PARTE DIARIO</button>
        </div>
...
export const handlePrintParteDiarioGlobal = (data, allSchedules) => {
  const win = window.open('', '_blank');
  const monthNames = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
  const currentMonth = monthNames[new Date().getMonth()];
    ...
    return `
      <div class="printable-page">
        ...
        <div class="schedule-section">
          <table class="schedule-table">
            <thead>
              <tr class="sched-date-row"><th style="border: none !important;"></th>${Array(5).fill('<th class="sched-date-cell"></th>').join('')}</tr>
              <tr><th style="width: 75px;">Horas/ Día/Fecha</th><th style="width: 18%;">Lunes</th><th style="width: 18%;">Martes</th><th style="width: 18%;">Miércoles</th><th style="width: 18%;">Jueves</th><th style="width: 18%;">Viernes</th></tr>
            </thead>
            <tbody>
              ${grid.map(row => {
                if (row.type === 'break' || row.isRecreo) {
        ...
        <div class="footer-suplentes">
          <div class="suplente-box">SUPLENTE N° 1</div><div class="suplente-box">SUPLENTE N° 2</div><div class="suplente-box">SUPLENTTE N° 3</div>
        </div>
      </div>
    `;
  }).join('');
  ...
  win.document.write(`
    <html>
      <head>
        <title>Partes Diarios Institucionales</title>
        <style>
          @import url('https://fonts.googleapis.com/css2?family=Roboto:wght@300;400;700;900&display=swap');
          ...
          .meta-row { font-size: 0.75rem; font-weight: 900; margin-bottom: 15px; }
          .attendance-container { display: grid; gap: 8px; margin-bottom: 25px; align-items: start; }
      ...
      <body>
        <div class="no-print-toolbar">
          <button class="btn-print" onclick="window.print()">🖨️ IMPRIMIR TODOS LOS PARTES</button>
        </div>
        ${coursesHTML}
      </body>
    </html>
...
export const handlePrintParteConInformacion = (data, course, scheduleData, selectedMonth, attendanceMap) => {
  const win = window.open('', '_blank');
